...
export class TaskRegistry implements IAmqpTaskRegistry {
  ...
  async pub(task: ITaskRequest): Promise<ITaskResult> {
    ...
    this.observables.set(task.id, obs);

    const result = new Promise((resolve, reject) => {
      obs.setOnComplete<ConsumeMessage>((msg) => {
        if (!this.config.consumer?.noAck) {
          this.channel?.ack(msg);
        }

        this.observables.delete(task.id);
